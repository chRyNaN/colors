 ...
 * [scRGB][ColorSpaces.LINEAR_EXTENDED_SRGB], are said to be linear because
 ...
 * instance, [scRGB][ColorSpaces.EXTENDED_SRGB] is used to manipulate the
...
internal constructor(
    ...
    init {
        if (primaries.size != 6 && primaries.size != 9) {
            throw IllegalArgumentException("The color space's primaries must be defined as an array of 6 floats in xyY or 9 floats in XYZ")
        }
    ...
    fun getPrimaries(/*@Size(min = 6)*/ primaries: FloatArray): FloatArray =
        this.primaries.copyInto(primaries)
    ...
    fun getTransform(/*@Size(min = 9)*/ transform: FloatArray): FloatArray =
        this.transform.copyInto(transform)
    ...
    fun getInverseTransform(/*@Size(min = 9)*/ inverseTransform: FloatArray): FloatArray =
        this.inverseTransform.copyInto(inverseTransform)

    override fun getMinValue(component: Int): Float = min

    override fun getMaxValue(component: Int): Float = max
     ...
     */
    /*@Size(3)*/
    fun toLinear(r: Float, g: Float, b: Float): FloatArray = toLinear(floatArrayOf(r, g, b))
     ...
     */
    /*@Size(3)*/
    fun fromLinear(r: Float, g: Float, b: Float): FloatArray = fromLinear(floatArrayOf(r, g, b))
    ...
    companion object {
        ...
        private val DOUBLE_IDENTITY: (Double) -> Double = { d -> d }
        ...
        private fun isSrgb(
            ...
            if (id == 0) return true
            if (!compare(primaries, ColorSpaces.SRGB_PRIMARIES)) {
                return false
            }
        ...
        private fun isWideGamut(
            ...
            return (
                    (
                            (
                                    area(primaries) / area(
                                        ColorSpaces.NTSC_1953_PRIMARIES
                                    ) > 0.9f && contains(
                                        primaries,
                                        ColorSpaces.SRGB_PRIMARIES
                                    )
                                    )
                            ) || (min < 0.0f && max > 1.0f)
                    )
